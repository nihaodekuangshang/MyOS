...
#define EFLAGS_IF 0x00000200  //将EFLAGS寄存器的IF位置1
...
struct gate_desc {
   ...
   uint16_t    selector;
   uint8_t     dcount;   //此项为双字计数字段，是门描述符中的第4字节。此项固定值为0，不用考虑
   uint8_t     attribute;  //属性  40~47位
...
static void init_pic()
{
   ...
   /*发送OCW1，让主片上IMR寄存器除第一位外都为1，打开主片上IR0的中断，即目前只接受时钟中断*/
 ...
 * 见kernel/kernel.S的call [idt_table + %1*4] */
        idt_table[i] = general_intr_handler; //默认处理函数为general_intr_handler
        intr_name[i] = "unknow";            //默认名称
    }
